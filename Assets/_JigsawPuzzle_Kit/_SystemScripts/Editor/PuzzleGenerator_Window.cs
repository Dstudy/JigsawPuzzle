...
public class PuzzleGenerator_Window : EditorWindow
{
    ...
    static int cols = 5;				// Puzzle grid columns number
	static int rows = 5;				// Puzzle rows columns number
	...
	static int pixelsPerUnit = 100;		// Sprites resolution
    ...
    public static PuzzleGenerator_Window Instance;
	...
	void CreatePuzzle () 
	{
		for (int i = 0; i < GenPuzzle.Instance.levelSize.Count; i++)
		{
			cols = GenPuzzle.Instance.levelSize[i];
			rows = GenPuzzle.Instance.levelSize[i];
			image = GenPuzzle.Instance.levelTexture[i];
			texturePath = "Assets/_Atlas"+ i +".png";
			...
			try
			{
				...
				slotControl = CreateAnotherGameObjects().AddComponent<SlotCtl>();
	...
	GameObject CreateAnotherGameObjects () 
	{
		// Load sprites assets
		Object[] spritePieces = AssetDatabase.LoadAllAssetsAtPath(texturePath);
		ArrayUtility.RemoveAt(ref spritePieces, 0);
        ...
        lvlindex++;
        ToUIElement(puzzle);
		puzzle.transform.SetParent(FindObjectOfType<Canvas>().transform);
		// Go through array and create gameObjects
		for (int y = 0; y < rows; y++) 
			for (int x = 0; x < cols; x++)
			{    
				...
				//Thêm slot để qua ly
				piece.AddComponent<PieceScroll>();
				
				if(puzzleGrid[x + y * cols].top == 1)
					piece.GetComponent<PieceScroll>().state = "up";
				if(puzzleGrid[x + y * cols].bottom == 1)
					piece.GetComponent<PieceScroll>().state = "down";
				if(puzzleGrid[x + y * cols].top == 1 && puzzleGrid[x + y * cols].bottom == 1)
					piece.GetComponent<PieceScroll>().state = "updown";
				if(puzzleGrid[x + y * cols].top != 1 && puzzleGrid[x + y * cols].bottom != 1)
					piece.GetComponent<PieceScroll>().state = "normal";
				// puzzle.GetComponent<SlotCtl>().slots.Add(piece.GetComponent<Slot>());
				piece.GetComponent<RectTransform>().pivot = new Vector2 (puzzleGrid[x + y * cols].pivot.x, puzzleGrid[x + y * cols].pivot.y);
				// piece.GetComponent<RectTransform>().pivot = new Vector2(0.5f, 0.5f);
			}

		puzzle.GetComponent<RectTransform>().localScale = 0.65f * Vector3.one;
			return puzzle;
	}
    ...
    private void ToUIElement(GameObject target)
    {
	    var rect = target.AddComponent<RectTransform>();
	    if (target.TryGetComponent(typeof(SpriteRenderer), out var spriteRendererComponent))
	    {
		    //Set size
		    ...
		    //rect.position *= scale;	
		    var spriteRenderer = (SpriteRenderer)spriteRendererComponent;
		    var imageComponent = target.AddComponent<Image>();
		    imageComponent.sprite = spriteRenderer.sprite;
		    var imgcolor = imageComponent.color;
		    imgcolor.a = 0;
